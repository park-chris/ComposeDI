...
import com.crystal.face_recognition.recognition.FaceAnalyzer.Companion.SMILE_SUCCESS_VALUE
...
internal class FaceAnalyzer(
    ...
    private val successListener = OnSuccessListener<List<Face>> { faces ->
        ...
        if (face != null) {
            if (detectStatus == FaceAnalyzerStatus.UnDetect) {
                ...
                listener?.detectProgress(50F, "오른쪽 눈만 깜박여주세요.")
    ...
    companion object {
        private const val EYE_SUCCESS_VALUE = 0.1F // 눈 감은거 판단 기준
        private const val SMILE_SUCCESS_VALUE = 0.8F // 웃는거 판단 기준
